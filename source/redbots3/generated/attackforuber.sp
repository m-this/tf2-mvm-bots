...
public Action CTFBotAttackUber_Update(BehaviorAction action, int actor, float interval, ActionResult result)
{
	...
	int target = FindEnemyNearestToMe(actor, MEDIC_ATTACK_UBER_SEEK_RANGE, false, true, false, TFClass_Unknown);
	if (target == -1)
	{
...
stock bool CTFBotAttackUber_IsPossible(int client, int medigun)
{
	...
	if (FindEnemyNearestToMe(client, MEDIC_ATTACK_UBER_SEEK_RANGE, false, true, false, TFClass_Unknown) == -1)
	{
		return false;
	}
	return true;
